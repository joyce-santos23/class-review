...
public class CreateFeedbackUseCase {
    ...
    public CreateFeedbackUseCase(FeedbackGateway feedbackGateway,
    ...
    public FeedbackResponseDTO execute(FeedbackRequestDTO request) {
        log.info("Creating feedback with rating {}", request.rating());

        Feedback feedback = new Feedback(request.rating(), request.comment());
